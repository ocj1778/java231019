...
public class ByteStreamApp {
	public static void main(String[] args) throws IOException {
		...
		while(true) {
			...
			// => 키보드를 누르면 키보드의 문자값(1Byte)이 입력스트림으로 전달
			//InputStream.read() : 입력스트림에 존재하는 하나의 문자값을 읽어 원시데이타(1Byte) 
			//로 얻어와 정수값(int)으로 반환하는 메소드
			// => 입력스트림에 문자값이 하나도 없는 경우 스레드는 일시 중지 상태로 전환
			// => 입력스트림에 엔터(Enter)가 입력되어 전달되면 일시 중지된 스레드는 재실행
			// => 입력스트림 또는 출력스트림 관련 메소드에서는 대부분 IOException 발생
			// => IOException : 입력스트림 또는 출력스트림에 문제가 있는 경우 발생되는 일반 예외
			readByte=System.in.read();
			
			//입력스트림 종료 신호(EOF - Ctrl+Z)가 전달된 경우 반복문 종료
			if(readByte == -1) break;
