...
//다중 스레드 프로그램의 문제점
// => 다수의 스레드가 같은 객체로 동일한 메소드를 호출하여 같은 명령들이 실행되는 경우 스레드로  
//전달되는 명령의 순서가 없어 잘못된 처리결과 발생 가능
//해결법)스레드를 동기화 처리하여 스레드에 대한 메소드 호출 제어

//스레드 동기화(Thread Synchronize) : 스레드가 메소드를 호출한 경우 메소드의 모든 명령들이 
//실행되기 전까지 다른 스레드로 메소드를 호출하지 못하도록 방지하는 기능
// => 스레드가 메소드를 호출하여 명령이 실행중인 경우 다른 스레드가 메소드를 호출하지 못하도록
//메소드에 장김(Lock) 기능 제공
// => 잠김 상태의 메소드를 다른 스레드가 호출하면 스레드을 일시 중지시킨 후 메소드의 잠김 
//상태가 해제되면 스레드로 메소드를 호출하여 명령 실행
...
public class AccountUserApp {
	public static void main(String[] args) {
		...
		*/
		...
		//이용하여 입금(출금) 처리 - 다수의 스레드로 입금(출력)을 동시에 처리 가능 
		userOne.start();
		userTwo.start();
		userThree.start();
	}
