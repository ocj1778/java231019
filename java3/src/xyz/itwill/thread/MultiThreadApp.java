...
//2.Thread 클래스 를 상속받은 자식클래스에서 무조건 run() 메소드를 오버라이드 선언
...
public class MultiThreadApp {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		// => Thread 클래스의 run() 메소드에는 명령이 하나도 없으므로 스레드는 생성된 후 바로 소멸하여 무의미
		...
		*/
		...
		// => 자식클래스에서 오버라이드 선언한 run() 메소드가 호출되어 스레드에 의해 명령 실행
		one.start();
		...
		*/		
		...
		//Runnable 인터페이스를 상속받은 익명클래스로 객체를 생성하여 Thread 클래스의 생성자에게 전달 
		...
		*/
		...
		//Runnable 인터페이스는 반환값도 없고 매개변수도 없는 함수형 인터페이스로 사용
		// => 람다표현식을 사용하여 익명클래스로 객체 생성 가능
		new Thread(() -> {
			for(int i=0;i<=9;i++) {
				System.out.print(i);
