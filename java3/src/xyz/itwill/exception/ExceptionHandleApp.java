...
// => 사용자가 프로그램을 정상적으로 실행하지 않거나 프로그램을 잘못 개발한 경우 예외 발생
...
//예외클래스(ExceptionClass) : 예외 관련 정보(예외 관련 메세지)를 저장하기 위한 클래스
...
//2.실행 예외 : 프로그램 실행시 예외가 발생될 가능성이 낮아 예외처리를 하지 않은 경우 컴파일시 에러 미발생
...
//      }
// => try 키워드의 블럭의 명령이 실행될 때 예외가 발생되면 JVM에 의해 예외클래스로 객체 생성
...
public class ExceptionHandleApp {
	public static void main(String[] args) {
		...
		try {
			...
			//프로그램 개발자에게 예외가 발생된 이유를 제공받아 출력(기록) : 에러 로그 - 필수
