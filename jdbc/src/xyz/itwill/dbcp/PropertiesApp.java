...
// => 파일의 확장자를 [properties]로 저장하며 파일에는 [이름 = 값] 패턴의 문자열을 행단위로 저장
// => Properties 파일에 제공되는 값은 이름으로 구분하므로 이름이 미중복되도록 작성
// => Properties 파일에는 영문자,숫자,일부 특수문자를 제외한 나머지 문자는 유니코드로 자동 변환되어 저장
// => 프로그램에서 자주 변경되는 값을 Properties 파일로 제공하면 프로그램에 대한 유지보수 효율성 증가
...
public class PropertiesApp {
	public PropertiesApp() throws IOException {
		...
		//전달받은 맵키(Key)에 대한 엔트리의 맵값(Value - 객체)를 반환하는 메소드
