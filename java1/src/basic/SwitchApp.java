...
// => 비교대상의 값을 case 키워드로 제공되는 값과 차례대로 비교하여 같은 값을 가지고 있는
//case 구문에 작성된 명령부터 하단에 작성된 모든 명령들을 실행
// => case 키워드로 제공되는 값은 리터럴 또는 상수(Constant)만 작성 가능
// => case 키워드로 제공되는 값이 중복되어 작성될 경우 에러 발생
// => 블럭 내부의 명령 중 break 명령을 실행하면 switch 구문 종료
// => 비교대상으로 제공되는 값이 case 제공되는 모든 값과 다른 경우 default 키워드로 제공되는 명령 실행 - 생략 가능 
