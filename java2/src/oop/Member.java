...
//this : 메소드 내부에서 메소드를 호출한 객체의 메모리 주소(HashCode)를 저장하여 객체를 참조해
//객체의 요소(필드 및 메소드)를 사용하기 위한 키워드
// => 메소드 내부에서 객체를 대신하기 필드 및 메소드를 사용하기 위해 제공되는 키워드
...
//상관없이 메모리(MethodArea)에 하나만 생성되어 동작 - 프로토타입 클래스(Prototype Class)
...
public class Member {
	...
	// => 일반적으로 생성자 내부에는 필드에 필요한 초기값을 저장하기 위한 명령 작성 - 초기화 작성
