...
//메소드는 자식클래스에서 사용 불가능
// => Java 프로그램은 하나의 클래스만을 상속받아 작성 - 단일상속
...
public class MemberEvent extends Member {
	...
	*/
	...
	//super : 자식클래스의 메소드에서 부모 객체를 참조하기 위한 키워드
	...
	public MemberEvent(String id, String name, String email) {
		...
		//자식클래스의 메소드에서는 this 키워드로 자식 객체를 참조하여 필드 또는 메소드를 사용하고
		//자식 객체에 필드 또는 메소드가 없는 경우 super 키워드로 부모 객체를 참조하여 필드 또는 메소드 사용
	...
	*/
	...
	//자식클래스에서 다시 선언
	// => 메소드를 오버라이딩 선언하면 부모클래스의 메소드(Hiding Method)는 숨겨지고 
	//자식클래스의 메소드만 사용
	//메소드 오버라이딩 선언 규칙 - 부모클래스의 메소드와 같은 접근제한자, 반환형, 메소드명
	//,매개변수, 예외 전달을 사용하여 자식클래스의 메소드로 작성
	/*
	public void display() {
