...
public enum Compass {
	...
	// => 상수필드를 선언하면 상수필드에는 상수필드의 첨자를 초기값이 자동 저장
	...
	// => 상수필드에 원하는 값을 저장하기 위해서는 반드시 매개변수가 존재하는 생성자 작성
	...
	// => 상수필드를 만들때 매개변수가 존재하는 생성자를 이용해야만 상수필드 생성 가능 
	// => 매개변수로 전달받은 값으로 상수필드에 필요한 초기값 저장
