...
//인터페이스로 객체를 생성할 수 없지만 클래스 선언시 인터페이스를 상속받아 사용 - 실체화 관계(Realization)
...
// => 인터페이스로 참조변수를 생성하여 자식클래스의 생성자로 만들진 객체 저장 가능 - 묵시적 
//객체 형변환에 의해 참조변수로 자식객체를 참조해 자식클래스의 메소드 호출
...
// => 클래스간의 결합도를 낮추어 시스템 교체에 따른 클래스 변경에도 기존 클래스에 대한 
//영향 최소화 - 유지보수의 효율성 증가
...
public class WolfHumanApp {
	public static void main(String[] args) {
		...
		//묵시적 객체 형변환에 의해 인터페이스로 생성된 참조변수로 자식객체를 참조하여 자식클래스의 
		//오버라이드 메소드 호출
		wolf.fastWalk();
		wolf.cryLoudly();
		System.out.println("==============================================================");
